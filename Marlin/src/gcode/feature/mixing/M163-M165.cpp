 ...
 *   //P[float]   The mix value from (0~10.0)
 *   P[int]     The mix value in percet from (0~100)
 */
void GcodeSuite::M163() {
  const int mix_index = parser.intval('S');  
  if (mix_index < MIXING_STEPPERS){
  #ifdef USE_PRECENT_MIXDATA
	mixer.set_collector(mix_index, (float)parser.intval('P')/10);
  #else
  	mixer.set_collector(mix_index, parser.floatval('P'));
  #endif
  }
  //auto normalize if the currrent channel index is the last steppers
  ...
  void GcodeSuite::M165() {
    ...
    MIXER_STEPPER_LOOP(i) {
      if (parser.seenval(mixing_codes[i])) {
        SBI(mix_bits, i);		
	#ifdef USE_PRECENT_MIXDATA
		mixer.set_collector(i, (float)parser.value_int()/10);
	#else
