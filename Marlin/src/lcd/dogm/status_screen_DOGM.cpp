...
#endif
    if (dodraw)
      _draw_centered_temp(target + 0.5, tx, 7);
  }
...
// Draw mix rate on the top
FORCE_INLINE void _draw_mixetruder_status()
{
  //draw mix rate data
  static char str[5];
  ...
  //draw mix data
  strcpy(str, i8tostr3rj(mixer.mix[0]));
  len = str[0] != ' ' ? 3 : str[1] != ' ' ? 2 : 1;
  lcd_put_u8str(x, MIX_DATA_Y, &str[3 - len]);
  x += len * 6;
  lcd_put_u8str(x, MIX_DATA_Y, "/");
  x += 5;

  strcpy(str, i8tostr3rj(mixer.mix[1]));
  len = str[0] != ' ' ? 3 : str[1] != ' ' ? 2 : 1;
  lcd_put_u8str(x, MIX_DATA_Y, &str[3 - len]);
  x += len * 6;
  lcd_put_u8str(x, MIX_DATA_Y, "/");
  ...
  strcpy(str, i8tostr3rj(mixer.mix[2]));
  len = str[0] != ' ' ? 3 : str[1] != ' ' ? 2 : 1;
  lcd_put_u8str(x, MIX_DATA_Y, &str[3 - len]);
  x += len * 6;
  ...
  strcpy(str, i8tostr3rj(mixer.mix[3]));
  len = str[0] != ' ' ? 3 : str[1] != ' ' ? 2 : 1;
  lcd_put_u8str(x, MIX_DATA_Y, &str[3 - len]);
  x += len * 6;
#endif
  ...
  if (mixer.gradient.enabled)
    lcd_put_u8str(MIX_STATUS_X, MIX_STATUS_Y, "gradient");
  else if (mixer.random_mix.enabled)
    lcd_put_u8str(MIX_STATUS_X, MIX_STATUS_Y, " Random");
  else
  {
    lcd_put_u8str(MIX_STATUS_X, MIX_STATUS_Y, "VTOOL:"); ///VTOOL//
    const char *toolstr = i8tostr3rj(mixer.get_current_vtool());
    const uint8_t len = toolstr[0] != ' ' ? 3 : toolstr[1] != ' ' ? 2 : 1;
    lcd_put_u8str(36, MIX_STATUS_Y, &toolstr[3 - len]);
  }
}
#endif // DO_DRAW_MIXEXTRUDER
...
#define _SD_INFO_X(len) (LCD_PIXEL_WIDTH - (len) * (MENU_FONT_WIDTH))
  ...
  if (first_page)
  {
    ...
    if (thermalManager.isHeatingHotend(e))
      SBI(new_bits, HEATBIT_HOTEND + e);
#endif
#if ANIM_BED
    if (thermalManager.isHeatingBed())
      SBI(new_bits, HEATBIT_BED);
#endif
#if DO_DRAW_CHAMBER && HAS_HEATED_CHAMBER
    if (thermalManager.isHeatingChamber())
      SBI(new_bits, HEATBIT_CHAMBER);
#endif
#if ANIM_CUTTER
    if (cutter.enabled())
      SBI(new_bits, HEATBIT_CUTTER);
#endif
    heat_bits = new_bits;
#endif
...
// Progress / elapsed / estimation updates and string formatting to avoid float math on each LCD draw
    ...
    if (ev != lastElapsed)
    {
      ...
      if (!(ev & 0x3))
      {
        ...
        if (!timeval && progress > 0)
          timeval = elapsed.value * (100 * (PROGRESS_SCALE)-progress) / progress;
        if (!timeval)
        {
          estimation_string[0] = '\0';
          estimation_x_pos = _SD_INFO_X(0);
...
#define CHAMBER_BITMAP(S) status_chamber_bmp
  ...
  {
    old_blink = blink;
    if (!thermalManager.fan_speed[0] || ++fan_frame >= STATUS_FAN_FRAMES)
      fan_frame = 0;
  }
#endif
...
//show mixing rate on ideal menu of LCD screen
#if 0 //MIXING_2_IN_1_OUT \
...
#else
      //show xy or Etotal on ideal menu
      if (showxy)
      {
...
void MarlinUI::draw_status_message(const bool blink)
{
  ...
  else
  {
    ...
    if (rlen < LCD_WIDTH)
    {
      ...
      if (--chars)
      { // Draw a second dot if there's space
        lcd_put_wchar('.');
        if (--chars)
