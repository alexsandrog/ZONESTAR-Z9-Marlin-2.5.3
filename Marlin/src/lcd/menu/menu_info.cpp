...
//
  ...
  void menu_info_printer() {
	...
	#endif
	#if ENABLED(OPTION_DM2208)
	  STATIC_ITEM_P(PSTR("TMC2208 MOTOR DRIVER"));
	#endif
	#if ENABLED(OPTION_DUALZ)
	  STATIC_ITEM_P(PSTR("DUAL Z DRIVER"));
	#endif
	#if ENABLED(OPTION_3DTOUCH)
	  STATIC_ITEM_P(PSTR("3DTOUCH SENSOR"));
	#endif
	#if ENABLED(OPTION_HIGHTEMP)
	  STATIC_ITEM_P(PSTR("HIGH TEMPERATURE"));
	#endif
