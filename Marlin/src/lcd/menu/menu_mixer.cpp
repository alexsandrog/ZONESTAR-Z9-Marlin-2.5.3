...
void lcd_mixer_random_z_start_edit() {
  ui.defer_status_screen();
  ENCODER_RATE_MULTIPLY(true);
  if (ui.encoderPosition != 0) {
    mixer.gradient.start_z += float(int32_t(ui.encoderPosition)) * 0.1;
...
void lcd_mixer_random_z_end_edit() {
  ui.defer_status_screen();
  ENCODER_RATE_MULTIPLY(true);
  if (ui.encoderPosition != 0) {
    mixer.random_mix.end_z += float(int32_t(ui.encoderPosition)) * 0.1;
    ui.encoderPosition = 0;
    NOLESS(mixer.random_mix.end_z, 0);
    NOMORE(mixer.random_mix.end_z, Z_MAX_POS);
  }

  if (ui.should_draw()) {
    char tmp[21];
    strcpy_P(tmp, GET_TEXT(MSG_END_Z));
    sprintf_P(tmp + strlen(tmp), PSTR(": %4d.%d mm"), int(mixer.random_mix.end_z), int(mixer.random_mix.end_z * 10) % 10);
    SETCURSOR(2, (LCD_HEIGHT - 1) / 2);
    lcd_put_u8str(tmp);
  }

  if (ui.lcd_clicked) {
    if (mixer.random_mix.end_z < mixer.random_mix.start_z)
      mixer.random_mix.start_z = mixer.random_mix.end_z;
    mixer.refresh_random_mix();
    ui.goto_previous_screen();
  }
}

void lcd_mixer_edit_randommix_menu() {
  START_MENU();
  BACK_ITEM(MSG_MIX);
  ...
  SUBMENU(MSG_START_Z, lcd_mixer_random_z_start_edit);
  MENU_ITEM_ADDON_START(9);
  sprintf_P(tmp, PSTR("%4d.%d mm"), int(mixer.random_mix.start_z), int(mixer.random_mix.start_z * 10) % 10);
  lcd_put_u8str(tmp);
  MENU_ITEM_ADDON_END();

  SUBMENU(MSG_END_Z, lcd_mixer_random_z_end_edit);
  MENU_ITEM_ADDON_START(9);
...
void menu_mixer() {
  ...
  EDIT_ITEM(uint8, MSG_ACTIVE_VTOOL, &v_index, 0, MIXING_VIRTUAL_TOOLS - 1, _lcd_mixer_select_vtool,true);
  {
    char tmp[10];
  ...
  {
    char tmp[13];
    SUBMENU(MSG_RANDOMMIX, lcd_mixer_edit_randommix_menu);
    MENU_ITEM_ADDON_START(10);
	  if(mixer.random_mix.enabled)
	    sprintf_P(tmp, PSTR(": enable"));
	  else
	  	sprintf_P(tmp, PSTR(": disable"));
      lcd_put_u8str(tmp);
    MENU_ITEM_ADDON_END();
  }
  #endif
...
void lcd_mixer_mix_edit() {  
  ...
  EDIT_ITEM_FAST_N(uint8, n, MSG_MIX_COMPONENT_N, &mixer.mix[n-1], 0, 100);
#else
  LOOP_S_LE_N(n, 1, MIXING_STEPPERS)
...
inline void _lcd_goto_mix_edit() {
  v_index = mixer.get_current_vtool();
  mixer.refresh_collector(10, v_index, mixer.collector);
#if ENABLED(USE_PRECENT_MIXDATA)
  mixer.copy_collector_to_mix();
#endif
