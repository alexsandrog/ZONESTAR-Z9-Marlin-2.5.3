...
#ifndef __AVR__ // || MIXING_2_IN_1_OUT
...
class Mixer {
  ...
  FORCE_INLINE static void stepper_setup(mixer_comp_t b_color[MIXING_STEPPERS]) {
   ...
   static inline void randommix_control(const float z) {
    ...
    static inline void update_mix_from_random() {
      float ctot = 0;
      MIXER_STEPPER_LOOP(i) ctot += random_mix.color[i];
      MIXER_STEPPER_LOOP(i) mix[i] = (mixer_perc_t)CEIL(100.0f * random_mix.color[i] / ctot);

      #ifdef MIXER_NORMALIZER_DEBUG
	    SERIAL_ECHOLNPGM("update_mix_from_random");
	  	SERIAL_EOF();
        SERIAL_ECHOPGM("random [ ");
        SERIAL_ECHOLIST_N(MIXING_STEPPERS, int(random_mix.color[0]), int(random_mix.color[1]), int(random_mix.color[2]), int(random_mix.color[3]), int(random_mix.color[4]), int(random_mix.color[5]));
        SERIAL_ECHOPGM(" ] to Mix [ ");
        SERIAL_ECHOLIST_N(MIXING_STEPPERS, int(mix[0]), int(mix[1]), int(mix[2]), int(mix[3]), int(mix[4]), int(mix[5]));
        SERIAL_ECHOLNPGM(" ]");
		SERIAL_EOF();
